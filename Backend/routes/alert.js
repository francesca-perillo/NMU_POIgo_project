...
router.post('/', async (req, res) => {
    ...
    const { title, description, photo, address, approval, location, GPSstreet, createdBy } = req.body;
    ...
    // Create a new alert
    const alert = await Alert.create({ title, description, photo, address, approval, location, GPSstreet, createdBy });
    res.send(alert);
});
...
router.put('/:id', async (req, res) => {
    ...
    const alert = await Alert.findByIdAndUpdate(id, { title, description, photo, address, approval, location, GPSstreet, createdBy }, { new: true });
    if (!alert)
...
router.patch('/:id', async (req, res) => {
    ...
    const { id } = req.params;
    // Retrieve values from the request body
    const { title, description, photo, address, approval, location, createdBy } = req.body;
